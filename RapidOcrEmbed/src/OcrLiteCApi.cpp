...
OcrDetect(OCR_HANDLE handle, const char *imgPath, const char *imgName, OCR_PARAM *pParam) {
    ...
    if (pParam) {
        ...
        Param = {50, 1024, 0.6f, 0.3f, 2.0f, 1};
    }
...
// Fill OCR text into table cells based on IoU matching
static void fillTableCellsWithOcr(std::vector<table::Table>& tables,
                                  const std::vector<TextBlock>& ocrBlocks,
                                  std::vector<std::array<int,4>>* outCells = nullptr) {
    for (auto& tbl : tables) {
        // Step 1: Fill all cells with OCR text
        for (size_t ri = 0; ri < tbl.rows.size(); ri++) {
            ...
            for (size_t ci = 0; ci < row.cells.size(); ci++) {
                ...
                for (size_t bi = 0; bi < ocrBlocks.size(); bi++) {
                    auto& block = ocrBlocks[bi];
                    ...
                    float cellArea = (cx1-cx0)*(cy1-cy0);
                    float ocrArea = (bx1-bx0)*(by1-by0);
                    float iou = inter / (ocrArea + cellArea - inter + 1e-8f);
        ...
        // Step 2: Merge content from duplicate cells (rowspan) to first occurrence
        for (size_t ri = 0; ri < tbl.rows.size(); ri++) {
            for (size_t ci = 0; ci < tbl.rows[ri].cells.size(); ci++) {
                auto& cell = tbl.rows[ri].cells[ci];
                if (cell.content.empty()) {
                    for (size_t rj = ri + 1; rj < tbl.rows.size(); rj++) {
                        if (ci < tbl.rows[rj].cells.size()) {
                            auto& laterCell = tbl.rows[rj].cells[ci];
                            if (laterCell.x1 == cell.x1 && laterCell.y1 == cell.y1 &&
                                laterCell.x2 == cell.x2 && laterCell.y2 == cell.y2 &&
                                !laterCell.content.empty()) {
                                cell.content = laterCell.content;
                                break;
                            }
                        }
                    }
...
// Generate HTML using the unified generateHTML function from img2table-cpp
static std::string buildTableHtml(std::vector<table::Table>& tables,
                                  const std::vector<TextBlock>& ocrBlocks,
                                  std::vector<std::array<int,4>>* outCells = nullptr) {
    fillTableCellsWithOcr(tables, ocrBlocks, outCells);

    std::string html;
    for (auto& tbl : tables) {
        html += table::generateHTML(tbl);
    }
    return html;
}

static std::string wrapFullHtml(const std::string& tableHtml, int imgW = 0, int imgH = 0) {
                    ...
                    if (h > 0) {
                        ...
                        if (tdStyleStart != std::string::npos && tdStyleStart < trEnd) {
                            ...
                            if (hErase != std::string::npos && hErase < trEnd) {
                                ...
                                if (hEnd < clean.size() && clean[hEnd] == ';') hEnd++;
    ...
    int totalW = imgW > 0 ? imgW : 720;
    int totalH = imgH > 0 ? imgH : 900;

    std::string result = "<!DOCTYPE html>\n<html>\n<head><meta charset=\"UTF-8\">\n"
        "<style>\n"
        "html, body { margin:0; padding:0; width:100vw; height:100vh; display:flex; "
        "justify-content:center; align-items:center; box-sizing:border-box; }\n"
        ".table-wrap { aspect-ratio:" + std::to_string(totalW) + "/" + std::to_string(totalH) + "; "
        "max-width:100%; max-height:100%; }\n"
        "table { border-collapse:collapse; table-layout:auto; width:100%; height:100%; }\n"
...
OcrDetectTable(OCR_HANDLE handle, const char *imgPath, const char *imgName, OCR_PARAM *pParam) {
    ...
    if (pParam) {
        ...
        Param = {50, 1024, 0.6f, 0.3f, 2.0f, 0};
    }
    ...
    if (pOcrObj->tableMode == 1) {
        ...
        pOcrObj->tableStrRes = wrapFullHtml(html, img.cols, img.rows);
    } else {
        ...
        pOcrObj->tableStrRes = wrapFullHtml(pOcrObj->tableResult.htmlStructure, img.cols, img.rows);
    }
    return TRUE;
}
